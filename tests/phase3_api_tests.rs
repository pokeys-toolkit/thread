...
fn test_enhanced_error_types() {
    ...
    match error {
        ThreadError::PinCapabilityError { pin, capability, suggestion, .. } => {
            assert_eq!(pin, 17);
    ...
    match error {
        ThreadError::HardwareConstraint { constraint, suggestion, .. } => {
    ...
    match error {
        ThreadError::ValidationError { message, context, recovery_suggestion } => {
    ...
    match error {
        ThreadError::ResourceConflict { resource, conflicting_operation, .. } => {
...
fn test_error_recovery_methods() {
    ...
    let error = ThreadError::pin_capability_error(
        50,
        "pwm",
        Some("Use pins 17-22 for PWM".to_string())
    );
...
fn test_bulk_operations_commands() {
    ...
    match cmd {
        DeviceCommand::SetPwmDutiesBulk { channel_duties: duties } => {
    ...
    let cmd = DeviceCommand::ReadAnalogInputsBulk { 
        pins: pins.clone() 
    };
